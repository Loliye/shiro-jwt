package com.mikufans.shiro.controller;

import com.github.pagehelper.PageHelper;
import com.github.pagehelper.PageInfo;
import com.mikufans.shiro.exception.CustomExcetption;
import com.mikufans.shiro.exception.CustomUnauthorizedException;
import com.mikufans.shiro.model.common.BaseDto;
...
import com.mikufans.shiro.model.valid.UserEditValidGroup;
import com.mikufans.shiro.model.valid.UserLoginValidGroup;
import com.mikufans.shiro.service.UserService;
import com.mikufans.shiro.util.AesCipherUtil;
import com.mikufans.shiro.util.JwtUtil;
import com.mikufans.shiro.util.RedisUtil;
import com.mikufans.shiro.util.UserUtil;
...
public class UserController
{
    ...
    public ResponseBean user(@Validated BaseDto baseDto)
    {
        ...
        {
            throw new CustomExcetption("查询失败(Query Failure)");
        }
    ...
    public ResponseBean online()
    {
        ...
        // 查询所有Redis键
        Set<String> keys = RedisUtil.keysS(Constant.PREFIX_SHIRO_REFRESH_TOKEN + "*");
        for (String key : keys)
        {
    ...
    public ResponseBean add(@Validated(UserEditValidGroup.class) @RequestBody UserDto userDto)
    {
        ...
        {
            throw new CustomExcetption("密码最多8位(Password up to 8 bits.)");
        }
        ...
        if (count <= 0)
        {
            throw new CustomExcetption("新增失败(Insert Failure)");
        }
    ...
    public ResponseBean update(@Validated(UserEditValidGroup.class) @RequestBody UserDto userDto)
    {
        ...
        if (count <= 0)
        {
            throw new CustomExcetption("更新失败(Update Failure)");
        }
    ...
    public ResponseBean delete(@PathVariable("id") Integer id)
    {
        ...
        if (count <= 0)
        {
            throw new CustomExcetption("删除失败，ID不存在(Deletion Failed. ID does not exist.)");
        }
    ...
    public ResponseBean deleteOnline(@PathVariable("id") Integer id)
    {
        ...
        {
            if (RedisUtil.delKey(Constant.PREFIX_SHIRO_REFRESH_TOKEN + userDto.getAccount()) > 0)
            {
                return new ResponseBean(HttpStatus.OK.value(), "剔除成功(Delete Success)", null);
            }
        }
        throw new CustomExcetption("剔除失败，Account不存在(Deletion Failed. Account does not exist.)");
    }
}